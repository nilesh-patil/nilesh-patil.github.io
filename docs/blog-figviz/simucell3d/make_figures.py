...
def figure_2(new, orig, static2):
    _scatter_fig(new, orig, static2, "cov",
                 "Workload imbalance never reaches the scheduler's thresholds",
...
def figure_6(orig, new):
    ...
    # two reference lines: 1x (no gain, solid) and 2x (dashed), labelled in a
    # thin header strip above the bars so both read clearly against the bars.
    ax.axvline(1.0, color=STONE3, lw=1.8, zorder=2)
    ax.axvline(2.0, color=STONE2, lw=1.5, ls="--", zorder=2)
    top = len(bands) - 1
    ax.text(1.0, top + 0.55, "1x", ha="center", va="center", fontsize=10,
            fontweight="bold", color=STONE3)
    ax.text(2.0, top + 0.55, "2x", ha="center", va="center", fontsize=10,
            fontweight="bold", color=STONE2)
    # call out RUN01's lone dip: the 250-499 band falls to 1.50x
    dip = (250, 499)
    if dip in bands and bo[dip]["speedup"]:
        dyi = y[bands.index(dip)]
        ax.annotate("RUN01 dips to 1.50x here,\nevery other band is 2.0-2.75x",
                    xy=(bo[dip]["speedup"], dyi + h / 2 + 0.02),
                    xytext=(2.5, dyi + 0.42), fontsize=9.5, color=INK_B,
                    va="center", ha="left",
                    arrowprops=dict(arrowstyle="->", color=INK_B, lw=1.2,
                                    connectionstyle="arc3,rad=-0.2"))
    ax.set_yticks(y); ax.set_yticklabels([BAND_LABELS[b] for b in bands], fontsize=10.5)
    ax.set_xlim(0, 3.3)
    ax.set_ylim(-0.6, top + 0.95)
...
# ---------------------------------------------------------------------------
# figure 7 (style C) : before/after dumbbell of per-phase iteration-time share
# ---------------------------------------------------------------------------
ORIG_STATIC_SHARES = {"contact": 82, "polar": 13, "timeint": 3, "mesh": 2}  # published, polar reconciled 14->13 so shares sum to 100
...
def figure_7(orig, new):
    fig, ax = fig_S(figsize=(13.4, 5.6), grid="x")
    ax.spines["left"].set_visible(False)
    so, sn = adaptive_shares(orig), adaptive_shares(new)  # RUN01 / RUN02 adaptive
    # one row per phase, top-to-bottom by Static (v1) share descending
    phases = [
        ("contact", PH_CONTACT, "Contact detection"),
        ("polar",   PH_POLAR,   "Polarization + internal forces"),
        ("timeint", PH_TIME,    "Time integration"),
        ("mesh",    PH_MESH,    "Mesh refinement"),
    ]
    ypos = list(range(len(phases)))[::-1]
    for yi, (k, color, _name) in zip(ypos, phases):
        before, after, repro = ORIG_STATIC_SHARES[k], so[k], sn[k]
        # stone connector: Static (v1) -> Adaptive (RUN01)
        ax.plot([before, after], [yi, yi], color=STONE, lw=1.6, zorder=1,
                solid_capstyle="round")
        # RUN02 reproduction: small filled square at its share
        ax.scatter([repro], [yi], marker="s", s=52, color=color, zorder=3,
                   edgecolor=CREAM, linewidth=0.6)
        # Static (v1): hollow circle (cream face, phase-colored edge)
        ax.scatter([before], [yi], marker="o", s=150, facecolor=CREAM,
                   edgecolor=color, linewidth=2.2, zorder=4)
        # Adaptive (RUN01): filled circle in the phase color
        ax.scatter([after], [yi], marker="o", s=130, color=color, zorder=5)
        # numeric labels, each on the outer side of its dot
        if before >= after:
            ax.text(before + 2.6, yi, f"{before:.0f}%", va="center", ha="left",
                    fontsize=11, color=INK_B)
            ax.text(after - 2.6, yi, f"{after:.0f}%", va="center", ha="right",
                    fontsize=11, color=color, fontweight="bold")
        else:
            ax.text(before - 2.6, yi, f"{before:.0f}%", va="center", ha="right",
                    fontsize=11, color=INK_B)
            ax.text(after + 2.6, yi, f"{after:.0f}%", va="center", ha="left",
                    fontsize=11, color=color, fontweight="bold")
    # headline mover: contact detection's drop, in points
    drop = ORIG_STATIC_SHARES["contact"] - so["contact"]
    ax.text((ORIG_STATIC_SHARES["contact"] + so["contact"]) / 2, ypos[0] + 0.30,
            f"-{drop:.0f} pts", ha="center", va="bottom", fontsize=11,
            fontweight="bold", color=INK_B)
    ax.set_yticks(ypos); ax.set_yticklabels([p[2] for p in phases], fontsize=12)
    ax.set_xticks([0, 20, 40, 60, 80])
    ax.set_xlim(-5, 92)
    ax.set_ylim(-0.6, len(phases) - 0.35)
    legend_handles = [
        Line2D([0], [0], marker="o", ls="", markerfacecolor=CREAM,
               markeredgecolor=STONE3, markeredgewidth=2.0, markersize=11,
               label="Static (v1)"),
        Line2D([0], [0], marker="o", ls="", color=STONE3, markersize=11,
               label="Adaptive (RUN01)"),
        Line2D([0], [0], marker="s", ls="", color=STONE3, markersize=9,
               label="Adaptive (RUN02)"),
    ]
    ax.legend(handles=legend_handles, fontsize=10.5, loc="lower right",
              framealpha=0.92)
    title_S(ax, "Contact detection stops dominating the iteration",
            "Share of mean iteration time per phase, Static (v1) to Adaptive (RUN01); "
            "squares mark the RUN02 reproduction. Cells > 100.",
...
def main():
    ...
    print("\nRendering figures...")
    # figure_1, figure_3, figure_4b, figure_5 remain defined but are not rendered.
