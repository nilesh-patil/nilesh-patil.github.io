---
layout: single
title: "Diffusion models - building an image generator from scratch, then the ladder to DALL·E 2"
...
<figure>
  ...
  <figcaption>The two chained processes. Going right along the top arrows is the fixed forward process: each step adds a touch of Gaussian noise, and the box outlines dissolve from solid to dotted as the image dissolves into static. Going left along the bottom arrows is the learned reverse process, where a single network applied over and over predicts the noise in its input and we subtract it; our training teaches that network to undo one step, and sampling chains it a thousand times.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>The forward process on one digit: at t=0 it is the clean image, and by t=999 it is indistinguishable from Gaussian noise. Notice how much structure survives even at t=400; that gradual dissolve is a deliberate choice of the noise schedule.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>U-Net structure, read left to right. The encoder shrinks the image down to the bottleneck, where the network settles the coarse, global shape, and the decoder grows it back into a full-resolution noise prediction. The three arcs on top are skip connections, each handing the decoder the same-resolution feature map from the encoder so fine detail travels straight across instead of being squeezed through the bottleneck. The timestep <code>t</code> becomes a sinusoidal embedding added into every block (the faint lines), which is what lets one set of weights behave differently at high noise and at low noise.</figcaption>
</figure>

Trained on MNIST for forty epochs, the loss falls fast and then crawls, and the crawl is fine, because diffusion loss values are a poor proxy for sample quality and the grids keep sharpening for a long time after the number itself has stopped moving.

<figure>
  <img src="/images/blog/diffusion-models-from-scratch/loss.svg" alt="Two smoothed training-loss curves against step, MNIST and Fashion-MNIST, each falling fast then flattening." loading="lazy">
  <figcaption>Noise-prediction MSE during training, smoothed. Both curves flatten early, but the sample grids keep sharpening for many epochs after the number stops moving, one of the awkward things about training generative models by this objective. The second curve is the Fashion-MNIST run from the next section, plotted alongside so both live in one figure.</figcaption>
</figure>
...
There is a catch I have been quiet about. Sampling ran the network a full thousand times to produce a single image, which is tolerable for a blog post and ruinous the moment you want more than a handful of them, so the field went looking for a shortcut and found one almost immediately, in [DDIM](https://arxiv.org/abs/2010.02502) (Song, Meng, and Ermon, late 2020).
...
Three of these rungs need no new experiments here - two are already folded into the build above, and one is a change of notation. DDIM is where a thousand sampling steps became fifty, which is what made everything downstream cheap enough to iterate on. [Improved DDPM](https://arxiv.org/abs/2102.09672) (Nichol and Dhariwal, 2021) gave me two things at once: the cosine schedule I used, and the trick of letting the network learn the reverse-step variance instead of fixing it. One function, better everything after. [Score matching](https://arxiv.org/abs/1907.05600) (Song and Ermon, 2019) is stranger. It looked like a separate idea until [Song and colleagues](https://arxiv.org/abs/2011.13456) showed in late 2020 that it is diffusion wearing different notation: predicting the noise is, up to a time-dependent scaling, the same as estimating the gradient of the data density, and both fall out of one continuous-time stochastic differential equation. So the words are interchangeable. When a paper says "score-based," read "diffusion."

**Beating GANs: classifier guidance (May 2021).** [Dhariwal and Nichol](https://arxiv.org/abs/2105.05233) tuned the architecture and added **classifier guidance**: at sampling time, nudge each step with the gradient of a classifier trained on noised images toward the class you want. The result beat the best GANs on ImageNet, which is the moment the field's default quietly flipped from GANs over to diffusion, though the catch in the title is real and worth stating plainly: this was one benchmark, ImageNet, measured on one carefully tuned setup. What guidance actually buys is a knob that trades sample diversity for fidelity, by sharpening the conditional distribution.
...
<figure>
  <img src="/images/blog/diffusion-models-from-scratch/conditioning_mnist.png" alt="Two rows of digits from the same seeds: the label-off row is ten unrelated digits, the label-on row is 0 through 9 in order." loading="lazy">
...
The last two rungs turned that steering into pictures from captions. Diffusing directly on megapixel pixels is brutally expensive, since the U-Net runs at full resolution a thousand times over, so [latent diffusion](https://arxiv.org/abs/2112.10752) (Rombach and colleagues) moved the whole process into the compact latent space of a pretrained autoencoder, cut the cost by roughly an order of magnitude, and wired in cross-attention so you can condition on text or layout. That is the architecture that would become Stable Diffusion later in 2022. As I write this, it does not exist yet. [GLIDE](https://arxiv.org/abs/2112.10741) had already shown classifier-free guidance beats CLIP-based guidance for following captions. Then [DALL·E 2](https://arxiv.org/abs/2204.06125), three weeks ago, restructured everything around CLIP: a prior (itself a diffusion model) turns the caption into a CLIP image embedding, and a diffusion decoder turns that embedding into a picture, so there is diffusion on both ends with classifier-free guidance doing the steering. The authors call it unCLIP, a different lineage from latent diffusion.
