---
layout: single
title: "Flow-network analysis - functional city structure from NYC yellow-taxi trips"
date: 2017-03-14T15:39:55-04:00
last_modified_at: 2026-02-17T22:14:07+05:30
categories: [blog]
...
header:
  overlay_image: /images/blog/headers/transportation-graph-nyc-taxi-data.jpg
  overlay_filter: 0.4
  teaser: /images/blog/headers/transportation-graph-nyc-taxi-data.jpg
---

Start with the payoff. There is an interactive map of New York where the East Village, sitting dead-center in Manhattan, lights up not like the dense core it geographically belongs to but like a far suburb the city's taxi flow keeps at arm's length. One directed graph, once I had built it, weighted it, and pushed on it from every angle I could think of, turned out to account for that reading and a dozen others like it. The rest of this post is how.
...
[Explore the interactive project]({{ site.baseurl }}/transportation-flow-network/){: .btn-soft .btn-soft--primary} [Browse the code on GitHub](https://github.com/nilesh-patil/transportation-flow-network){: .btn-soft}

In 2015, New York City logged 146 million yellow-taxi trips. The whole of what follows is one object built out of them, looked at from a series of different angles. I build the directed graph and weight its edges. I let it partition the city, then subtract out what volume and geography already predict so the residual map above can speak. I re-time the same flows by hour, stress-test the structure under node removal and against null models, and finally re-run the pipeline across a decade to see whether the shape outlasts the volume. That last question is the point of the whole exercise: the volume of trips is weather, the geometry underneath is climate, and it is the climate I am after. Every section is a single mutation of that one graph rather than a separate finding standing alone, so if the suburb map is the part you came for, the rest is the machinery that earns it.

One scope note before the arrows. This is yellow taxis only, which is one observation channel into how the city moves, not all of it, and by 2015 ride-hailing and green cabs already carried a growing slice of trips, so everything below is a claim about yellow-taxi flow, not about all travel. That single bias shapes how I read the flagship map you just saw and the decade panel that closes the piece, so I carry it the whole way through.
...
The Taxi and Limousine Commission distributes the trip dataset publicly, one row per trip, with a pickup point, a dropoff point, distance, fare, and timestamps<sup>[1]</sup>. The whole post rests on one move. A trip becomes a directed edge from its pickup node to its dropoff node, and every trip between the same pair collapses into one weighted edge whose weight is the trip count, so an entire year of individual rides settles into a graph small enough to hold in the head and structured enough to interrogate.
...
The node is a taxi zone by design. TLC distributes its trips pre-aggregated to 263 zones, so a zone is the natural unit of the graph, and a single year resolves to a few hundred nodes rather than a few hundred thousand raw coordinates, which is the difference between an object you can partition and stress-test and a point cloud you can only sample.
...
A directed graph has hubs. Rank the zones by in-flow and by out-flow and ask whether the two orderings match. They do not.
...
The two curves are heavy-tailed, and they are not mirror images of each other. That asymmetry is the first real structure in the data, invisible in the raw rows but immediate once the trips are a graph. It sets up two things worth holding onto: the hubs come in two opposite kinds, and those heavy tails will later face a power-law test they fail.
...
Transport hubs are the network hubs, office districts next. The more interesting reading is that two zones can pull the same large volume in completely different ways, and what separates them is trips per distinct source: in-strength divided by the number of distinct origins a zone draws from, cheap to compute off the weighted edge table and enough to split hubs the eye would lump together.
...
Running Leiden community detection on the same weighted, directed graph splits the city into four functional districts, the substrate clustering itself rather than me imposing a grouping. (Leiden was published in 2019, after the original analysis; this partition, like the decade panel at the end, comes from the later re-run of the pipeline.)
...
Four is what falls out at the resolution I used, and the count is resolution- and seed-dependent, since a coarser resolution merges districts, a finer one splits them, and a sweep across resolutions returns anywhere from four to seventeen. What is robust is the structure at this scale. Over 100 random seeds the count holds in the range three to four, with a mode of four and a mean Adjusted Rand Index of 0.751 between seed pairs, so the partition is stable to the stochasticity of the algorithm. Modularity is Q = 0.189, which sits at z = 13.8 above a weight-reshuffle null, so the districts are real concentrations of flow rather than an artifact of the method, and about half the trips stay inside a single district, while the partition agrees poorly with both borough and service-zone labels, which is exactly the point: taxi connectivity draws a different map than administration does.

Naming districts is descriptive. The sharper question is where Manhattan functionally ends, and answering it needs a model of the flow the graph already predicts, so that whatever is left over is genuinely the part volume and geography cannot explain.

## Subtracting the obvious: where Manhattan ends

The flagship question, the one the map at the top answered before I had shown any method, is whether a central neighborhood can behave, in taxi flow, like a far suburb. Getting from the substrate to that map takes three beats: how flow decays with distance, whether a gravity model fits, and which zones the model cannot explain once you subtract everything it can.
...
The model I lean on is a doubly-constrained gravity model, the Furness form, which conditions on only two things: each zone's own marginals, both what it sends and what it receives, and the distance between every pair of zones. Because it knows nothing about a pair beyond those two constraints, the residual it leaves is non-circular. It is precisely the slice of flow that volume and geography alone do not explain. The calibrated decay exponent is 1.146. An unconstrained Poisson fit returns a gentler exponent of 0.72, but it over-reconstructs the flows by leaning on the data it is supposed to explain, so I report it only as an exponent and never as evidence the gravity story holds.

The fit holds across the core, so the subtraction is well-founded before I read anything off it, and plotting observed flows against the model's predictions, the common part of commuters lands at 0.816, a fair description of the city before any residual is read.
...
<figure>
<img src="{{ site.baseurl }}/images/blog/transportation-graph-nyc-taxi-data/08_where_manhattan_ends.png" alt="Manhattan-core taxi zones colored by gravity-model deviance residual, under-connected zones blue and over-connected red" />
...
Split the flow by daypart, that is, build the same graph but on morning trips and evening trips separately, and the directional structure flips sign. A net-flow index, trips out minus trips in, normalised, runs Midtown Center as a sink in the morning at -0.567 and a source in the evening at +0.128. That is the classic central business district, filling up at 9am and emptying out at 6pm, while the East Village is the mirror image, a source in the morning at +0.492 and a sink in the evening at -0.168.
...
Those are the views I came for. A comprehensive read has to ask the harder network-science questions the views above skip, and several have tempting wrong answers; the density of this graph is what makes them tempting, and it is here that the easy claims get tested and several fail.

**The heavy tails are not a clean power law.** The strength plots beg the scale-free question, so I ran it properly: the Clauset-Shalizi-Newman recipe of discrete maximum-likelihood fits, a goodness-of-fit bootstrap, and Vuong model comparisons.
...
The verdict is heavy-tailed but not scale-free. Out-, in-, and total-strength fit power-law exponents of 1.23, 1.35, and 1.33, all below 2, but the goodness-of-fit bootstrap rejects the power law outright for each (p_gof = 0) and every one ties statistically with a lognormal under a Vuong test (p = 0.65, 0.90, 0.59). They beat an exponential decisively, and that is all I will claim. The in-degree fit runs into the N-1 ceiling on a near-complete graph and says nothing, and with only 262 nodes the tail is too short to support a scale-free label anyway. It is tempting and wrong; I do not use it.
...
The naive answer is a trap. The graph is too dense to disconnect by random removal (Molloy-Reed kappa = 365.5 against a threshold of 2, a degenerate baseline of a near-complete graph, not a resilience finding). The axis that matters is surviving trips. At a 10% removal fraction, random removal keeps 82.2% of trips and about 96% of weighted efficiency, while a strength-targeted attack, recomputing strengths after each removal, keeps only 13.0% of trips and about 17% of efficiency, and the unweighted largest-component curve stays near 0.90 under every strategy, blind to the gap. Robust to random failure, fragile to targeted hub removal, and you only see it if you weight by flow.
...
The decisive test is a weight-preserving null: it fixes the exact observed topology and degree sequence and only permutes the trip weights across the fixed edges. Against it, the observed weighted reciprocity of 0.818 sits at z = +217 and the cost-weighted efficiency at z = -54, so where the heavy flows sit, the mutual Midtown and airport corridors, is wildly non-random. The Erdos-Renyi and configuration topology nulls are degenerate on a graph this dense, their z-scores huge but uninformative, so I do not cite them. The structure that matters lives in the weights, not the wiring.

**The layout is geometrically near-optimal.**
...
Corridor circuity, the ratio of network to straight-line distance, has a mean of 1.006 and a median of 1.000; global efficiency is 0.173, which is 0.997 of the straight-line ideal; mean straightness is 0.997<sup>[5]</sup>. The Manhattan grid and the Midtown corridors are essentially direct. The betweenness load is concentrated (Gini 0.89, top-10 zones carrying 54.7%) but not spatially clustered (Moran's I = 0.028, p = 0.238), so it sits in scattered hubs rather than one contiguous patch. A Motter-Lai cascade triggered at JFK collapses the giant component at every tolerance, but that is partly an artifact: 149 of 259 nodes carry zero initial load, so any rerouting fails them.

Every number so far is one year. The last move is to rebuild the graph once a year across a decade and ask whether the shape survives a demand collapse the volume does not.
