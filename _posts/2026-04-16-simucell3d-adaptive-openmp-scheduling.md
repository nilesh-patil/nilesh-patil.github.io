...
That idle third is invisible to `top` because it reports *aggregate* CPU time. 800% across eight cores looks identical whether one thread sprints while seven idle or all eight share the load evenly. Only a per-thread trace tells you which. If you train models, you already know this failure by another name: a data-parallel step where one worker drew all the long sequences while the rest sit idle at the all-reduce barrier.

**What this is, and what it is not.** This is a writeup of my [fork](https://github.com/nilesh-patil/simucell3d) of SimuCell3D. Past a few bug fixes to the original, it covers three things: where the load imbalance came from, how I measured it with one cheap number, and what it took to make the scheduler adapt as the tissue grows. It is *not* a clean scheduler-only ablation. The same fork also rewrote contact detection, and I have not separated the two effects, a caveat I return to once, in full, near the end. It is also a single-node, eight-core story, with no distributed or MPI scaling. The post runs long, so treat the section headers as signposts and skip to "The numbers" if that is what you came for.

> **TL;DR.** Static OpenMP scheduling handed every thread an equal slice of cells regardless of per-cell cost, so whichever thread drew the heavy cells kept them while the rest idled at the barrier. I built a cheap workload estimator, wired it to per-loop schedule selection, and let it adapt as the tissue grows. On the same eight cores and stopping point, adaptive reached **19,958 cells** to v1's **9,693**. At matched cell counts, median adaptive throughput runs **~2.0-2.6x higher, peaking near 2.75x** across most of the range. The one caveat: this run bundles the scheduling change with a contact-detection rewrite, so it is not a clean scheduler-only measurement.
...
<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-1.svg" alt="Thread imbalance versus tissue size on a log scale: Static v1 stays high, Adaptive sits consistently lower.">
  <figcaption>Workload imbalance versus tissue size (log scale), Static (v1) against Adaptive. The <code>thread_imbalance_pct</code> column is the workload CoV scaled to a percent, a proxy for how uneven per-cell cost is, not a direct barrier-wait measurement. Adaptive stays below Static across the whole range: Static mean 16.4%, median 16.1%, max 21.2%; Adaptive mean 10.6%, median 12.2%, max 14.4%.</figcaption>
</figure>
...
<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-2.svg" alt="Workload CoV versus tissue size for Static v1 and Adaptive, with dashed 0.4 and 0.6 threshold lines that neither curve reaches." loading="lazy">
  <figcaption>The same imbalance signal as figure 1, now in raw CoV and plotted against the scheduler's decision lines. Workload CoV versus tissue size, Static (v1) against Adaptive. Neither mode reaches the 0.4 or 0.6 chunk-band thresholds (dashed, from <code>calculate_optimal_chunk_size()</code>), so the adaptive chunk divisor stays at its coarsest setting throughout. Adaptive: mean 0.106, median 0.122, max 0.144. Static: mean 0.164, median 0.161, max 0.212.</figcaption>
</figure>

Before the design, one property of this run: adaptive-mode CoV peaked at 0.144 and never reached the chunk-band thresholds (0.4 and 0.6) built into the scheduler. The chunk divisor stayed at 4, the coarsest setting, for the entire experiment. The high-CoV dynamic-chunking regime the system is designed for was never exercised on this run. That makes the run a strong stress test for cell count, but not for the scheduler's high-heterogeneity branch.
...
In the maximum-cell fast-growth run, `grep GROWTH` against the adaptive performance-diagnostics log returns zero matches. GROWTH never fired, even on the run that reached the largest tissue.

<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-3.svg" alt="Semi-log scheduler phase plot: INITIALIZATION through iteration 4,100, then HOMEOSTASIS out to 19,958 cells, with GROWTH never appearing." loading="lazy">
  <figcaption>Scheduler phase decisions across the maximum-cell fast-growth run (adaptive mode), iteration on a linear axis and cell count on a log axis. INITIALIZATION fires through iteration 4,100 while the tissue has fewer than 10 cells. The tissue then jumps straight to HOMEOSTASIS and stays there from 11 to 19,958 cells. GROWTH never appears.</figcaption>
</figure>
...
The headline is cell count. Under the same compute budget - same stopping point, eight cores - the maximum-cell run reached roughly twice as many cells in adaptive mode: about 20k against 10k in v1.0.

<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-4-wallclock-cell-growth.svg" alt="Cell count versus wall-clock time on log2-log2 axes: adaptive reaches about twice the cells of v1 at the same stopping point." loading="lazy">
  <figcaption>Cell count at the same stopping point on 8 cores, both axes log2 so every gridline is one doubling. The comparison log ends at 19,912 adaptive cells against 9,693 v1 (the adaptive diagnostic endpoint records 19,958; the checkpoint near the stop, 19,614 against 9,399): about 2x more cells for the same budget. On these axes adaptive growth is a near-straight line, cells proportional to t^0.88.</figcaption>
</figure>

The same run with **linear** time on the x-axis makes the doubling time itself legible:

<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-4b-doubling-semilog.svg" alt="log2 cells versus linear wall-clock hours: the curve bends upward because each doubling takes longer, with adaptive above v1 throughout." loading="lazy">
  <figcaption>The same data, log2 cells against linear time. A straight line would mean a constant doubling time; the curve bends because each doubling takes longer, roughly 2x the wall-clock of the one before. For adaptive, one doubling stretches from about 3 hours at 2k-4k cells to about 15 hours at 8k-16k. Its curve stays above v1's throughout, so it reaches each rung sooner.</figcaption>
</figure>

Throughput tells the same story from the rate side, iterations per second as the tissue grows:

<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-5.svg" alt="Iterations per second versus cell count on log-log axes: adaptive continues well past where v1 terminates." loading="lazy">
  <figcaption>Iterations per second versus cell count (log-log), from the maximum-cell run. Adaptive uses per-iteration computational diagnostics; v1 uses the cleaned monitor data from the same benchmark. v1 terminates at 9,693 cells; adaptive continues to 19,958.</figcaption>
</figure>

At matched cell counts the picture is cleaner than the raw endpoint IPS: adaptive is faster in every usable band after 25 cells, with one midrange dip around 250-499 cells.

<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-6-speedup-by-cell-band.svg" alt="Matched-cell speedup by cell-count band: mostly 2.0 to 2.75x, with one dip near 250-499 cells." loading="lazy">
  <figcaption>Matched-cell speedup from the maximum-cell run, binned by cell count: median adaptive IPS divided by median v1 IPS in the same band. The 10-24 cell band is omitted because v1 has only one cleaned sample there.</figcaption>
</figure>

The scaling exponent (time per iteration ~ N^α) is less clean here because the v1 side comes from monitor-derived cleaned data rather than per-iteration diagnostics. Adaptive gives α = 1.133 (R² = 0.999) for cells ≥ 10. The v1 monitor fit gives α = 1.132 but with much weaker fit quality (R² = 0.835), so I would not lean on the exponent comparison as primary evidence. The matched-cell speed bands are the more defensible summary.

The physics barely moves: median pressure deviation between the two modes is 2.93% across 389 matched iterations in the cleaned biological outputs. Reordering the work changes only how fast the simulation reaches that answer.
...
<figure>
  <img src="{{ site.baseurl }}/images/blog/simucell3d/figure-7.svg" alt="Share of mean iteration time by phase: contact detection drops from 82% under Static to 54% under Adaptive." loading="lazy">
  <figcaption>Share of mean iteration time by phase (cells > 100), Static (v1) against Adaptive. Static: contact detection 82%, polarization plus internal forces 14%, time integration 3%, mesh refinement 2%. Adaptive: contact detection 54%, polarization plus internal forces 33%, time integration 8%, mesh refinement 5%.</figcaption>
</figure>

Contact detection drops from ~82% of mean iteration time to ~54%. Polarization and internal forces become the second-largest phase, but only because contact detection got faster, not because that work grew. Time integration stays a minor 3-8%, never the dominant cost.
