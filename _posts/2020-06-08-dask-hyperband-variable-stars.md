...
This is multinomial logistic regression, fit by SGD. Tuned, it lands in the low 90s, 91.6% to 92.5% on the held-out test set across the two searches below, a few points under the forest's 96.8%. But it has the two properties the forest lacks. It is genuinely sensitive to its regularisation: `alpha` and `l1_ratio` swing its validation accuracy from the high 80s up to the 94.1% of the best pair the search found, so the tuning is not cosmetic. And it learns *incrementally*, one `partial_fit` pass at a time, so I can train it a little, look at how it is doing, and decide whether to keep going. That second property is the whole game here.
...
The fix is an old idea with a good name: successive halving. Run the search like a knockout tournament. Every configuration plays a first round on a tiny budget, say one epoch. Look at the scores, keep the top third, and knock the rest out. The survivors advance to a round with three times the budget, and you look and cut again. After a few rounds only a handful of configurations are still standing, and those are the ones trained to the full 81. A configuration's total compute is just how far it got before it lost, so the ones with no chance cost almost nothing.
...
<figure>
  ...
  <figcaption>Successive halving as a knockout tournament. Every configuration starts with one epoch (left). Each rung is an elimination round: the worst two thirds are knocked out (the faded bars) and the survivors advance to three times the epochs (the taller bars), until a few reach the full budget on the right. The compute spent on any configuration is proportional to how many rounds it survived.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>The whole tournament, one line per configuration: validation accuracy against how many epochs it was allowed. The grey lines are the models knocked out early: most ran one to three epochs (95 of the 133 dropped) and scored across a wide band, mostly in the high 80s to low 90s, short of the survivors. The blue lines advanced round by round to the full 81 epochs. Almost all the compute lives in the handful of blue lines stretching to the right; the grey crowd on the left cost a few epochs each and nothing more.</figcaption>
</figure>
...
<figure>
  <img src="{{ site.baseurl }}/images/blog/dask-hyperband-variable-stars/accuracy_vs_budget.png" alt="Accuracy against training passes for both searches: Hyperband sits left at far less compute, tied with random search on test and higher on validation.">
  <figcaption>Accuracy against the compute each search spent. On the test set the two land 0.9 of a point apart (the shaded band), a tie inside one seed's noise. On the validation score Hyperband optimises, its pick is the higher of the two, 94.1% against 92.8%; the wider drop to its 91.6% test is the cost of picking the winner on a small validation split. Hyperband reaches that neighbourhood at 1,581 passes, random search at 11,583.</figcaption>
</figure>
...
The other limit is scale. On this problem the absolute saving is seconds, because an SGD epoch on four thousand rows is nearly free. The reason to care is what happens when an epoch is *not* free: tuning a gradient-boosted model with hundreds of trees, or a neural net with real epochs, where a single bad configuration trained to the end costs minutes. It is the same tournament, but now every epoch a doomed configuration burns is minutes you do not get back, so eliminating the losers early is the whole point. There the 7.3x is minutes saved per search, and the same `HyperbandSearchCV` call scales out across a cluster without changing a line. That is the case I would actually reach for it, and the one I will try next.

[dask-experiments on GitHub](https://github.com/nilesh-patil/dask-experiments){: .btn-soft .btn-soft--primary} [Part one: the feature loop]({{ site.baseurl }}/posts/dask-parallel-light-curves/){: .btn-soft} [When the model itself outgrows one machine]({{ site.baseurl }}/posts/distributed-kmeans-clustering/){: .btn-soft}
