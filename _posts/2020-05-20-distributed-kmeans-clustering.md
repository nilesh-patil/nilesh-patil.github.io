---
layout: single
title: "distributed k-means - clustering datasets that outgrow one machine"
...
header:
  overlay_image: /images/blog/headers/distributed-kmeans-clustering.jpg
  overlay_filter: 0.4
  teaser: /images/blog/headers/distributed-kmeans-clustering.jpg
---
...
The failure that matters here is a specific one: the loop never reaches its first iteration, because the data does not fit in memory. Everything below follows from that.
...
The cost of that loop is $O(n \times k \times d \times i)$, where `n` is the number of points, `k` is the number of clusters, `d` is the dimensionality, and `i` is the number of iterations. The term that hurts is `n`, because every one of those `n` points gets touched `k` times on every iteration. When `n` is a billion and the array will not fit in memory, the loop cannot even begin. That is the wall.

Now look at where the work lives. The assignment step is the expensive one, and each point is assigned independently of every other point. To decide which centroid a point belongs to, you need that point and the current `k` centroids, and nothing else. That independence is the entire reason distributing k-means works.

So you pin the data where it already lives. You split the billion points across workers. Each worker holds a partition, and that partition never moves. Then you broadcast the `k` centroids to every worker. Each worker assigns its own partition locally, which is embarrassingly parallel, and instead of shipping points anywhere, it emits a tiny summary per cluster: the sum of its points and a count. A tree-reduce combines those partial sums and counts into `k` updated centroids, those centroids broadcast back out, and the next iteration begins.

Each iteration, only the `k` centroids cross the wire. For our billion points in 8 dimensions, that is a handful of tiny vectors moving instead of the entire dataset. The data stays pinned. Only the summaries travel.
...
<figcaption>One iteration of distributed k-means. Partitions never move. Each worker assigns its own points and emits per-cluster sums and counts, a tree-reduce folds them into k updated centroids, and those k centroids broadcast back out to all three workers.</figcaption>
</figure>
...
Classic k-means++ picks seeds one at a time. Each new centroid is chosen with probability proportional to its squared distance from the seeds chosen so far. So you have to scan the whole dataset before you can pick the next seed. That is `k` sequential passes over the data. On one machine, fine. On a cluster, each of those passes is a full distributed scan, effectively `k` full distributed passes stacked back to back, all of them before the actual clustering loop begins. Line the two costs up. Seeding is `k` distributed passes; the clustering loop that follows runs `i` iterations, and `i` is typically a few tens. So once `k` climbs into the thousands, the seeding passes dominate by construction, and you can spend longer seeding than clustering. Push `k` into the low thousands and seeding alone is a few thousand full distributed scans.

The fix is **k-means||** (k-means parallel), from [Bahmani et al. 2012](https://arxiv.org/abs/1203.6402), the "Scalable K-Means++" paper. Instead of drawing one seed per pass, it oversamples: each pass draws several candidate centers at once, still with probability proportional to squared distance. Bahmani et al. show that each oversampling pass cuts the expected remaining cost by roughly a constant factor, and a quantity that shrinks by a constant factor each round reaches its target in a logarithmic number of rounds. So the candidate pool fills in a logarithmic number of passes instead of one seed at a time. It then reduces that pool down to `k` final seeds with one local reduction, no further distributed scans. Same goal as k-means++, good well-spread seeds, but the passes over the distributed data drop from `k` to a logarithmic count. That count is small in practice: Spark's k-means|| defaults to just two oversampling steps (`initSteps=2`).
...
<desc>On the left, k-means++ runs k sequential full passes over distributed data, one seed per pass, drawn as a tall stack of repeated scans. On the right, k-means parallel runs a few oversampling passes, logarithmic in count, that gather a candidate pool, then one local reduction down to k seeds.</desc>
...
<text x="400" y="44" font-size="11" stroke="none" fill="currentColor" opacity="0.7">logarithmic oversampling passes</text>
...
<figcaption>Why k-means|| wins at scale. k-means++ needs k full distributed passes, one seed each. k-means|| oversamples in a logarithmic number of passes into a candidate pool, then reduces locally to k seeds with no further scan.</figcaption>
</figure>

This is why Spark's MLlib defaults to k-means||, swapping the seeding step so the passes over distributed data stay logarithmic rather than scaling with `k`:

```python
from pyspark.ml.clustering import KMeans as SparkKMeans

# k-means|| (k-means parallel) is the scalable seeding here, over k-means++.
# Both aim for good seeds, but k-means|| needs only a logarithmic
# number of passes, where k-means++ needs k sequential ones.
...
PySpark MLlib and Dask-ML both implement the same data-movement model from the first section. The difference is the ecosystem each one lives in.

Reach for **Spark** when the data already lives in the Spark or JVM ecosystem and is huge. You load it, pack the columns into a single `features` vector with `VectorAssembler`, and fit. Spark handles the partitioning and the reduce:
...
There is a sharp edge here, because the obvious move does not work. `dask_ml.cluster.KMeans` (I am on dask-ml 1.4.0) does **not** expose `partial_fit`, so reaching for it fails immediately:
...
That class implements the full-dataset k-means|| fit, not the incremental API. For a stream you drive yourself, drop dask-ml here and use scikit-learn's `MiniBatchKMeans` directly, which does support `partial_fit`:
...
def streaming_kmeans(data_stream, k=3):
    ...
    for batch in data_stream:
        model.partial_fit(batch)   # this batch updates the centroids, then is dropped
    return model
```

When you own the loop, that is all you need, and wrapping it in `dask_ml.wrappers.Incremental` buys nothing. `Incremental` earns its keep in the other regime: the data is already one large Dask array, and you want `partial_fit` mapped over its blocks for you rather than writing the loop.
...
Each batch nudges the centroids a little, then is discarded, so you never hold the whole stream. This is the regime where even "fits across the cluster" stops being true, because there is no fixed dataset to fit, only an arrival rate.
...
One storage habit matters far more on a cluster than on a laptop: store features as `float32` rather than `float64` when you can. For our billion 8-dimensional points that halves the footprint from about 64 GB to 32 GB, which is often the difference between a partition fitting in a worker's memory and spilling to disk. That ties straight back to the stake at the top, because the whole reason you reached for a cluster was that the data stopped fitting. Standardizing and dropping outliers first (z-score, keep `z_scores < 3`) still helps the clustering itself, the same way it does on one machine.

## The pattern, and the limit

Strip away the engines and the same shape sits underneath: figure out which step is embarrassingly parallel, run that step where the data already sits, and ship as little as possible per round. Here the assignment step parallelizes per partition, the update step is a tree-reduce of partial sums, and the only thing crossing the wire each iteration is the `k` centroids.

<figure>
<svg viewBox="0 0 640 300" role="img" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;color:inherit" fill="none" stroke="currentColor" stroke-width="2">
<title>Decision: distribute k-means or not</title>
<desc>A flow that starts by asking whether the data fits in one machine's RAM. If yes, use single-node MiniBatchKMeans and skip the cluster. If no, distribute, then branch on Spark for JVM or huge data, Dask for NumPy or Pandas native workflows, or Incremental for streaming data that never fully lands.</desc>
...
<text x="120" y="178" font-size="10" stroke="none" fill="currentColor" text-anchor="middle" opacity="0.7">no cluster needed here</text>
...
<figcaption>How to decide. The first question is whether the data fits in one machine. If it does, stay on a single-node MiniBatchKMeans and skip the cluster. Only when it does not do you distribute, and then the choice is ecosystem: Spark, Dask, or Incremental for streams.</figcaption>
</figure>

That same shape, parallelize the per-record step, reduce small summaries, broadcast a small model, recurs across a lot of distributed machine learning. The centroid tree-reduce is the same all-reduce that sharded SGD uses to sum its per-worker gradients before taking the next step; distributed gradient boosting and federated training lean on the same move. K-means is just the version where the model is small enough that you can watch the trick happen.

And none of it is free. You pay shuffle cost on every reduce and coordination cost across workers. That overhead is a fixed cost you pay before any clustering happens, so on data that fits in RAM a cluster can only lose to a single-node `MiniBatchKMeans`: it pays a startup cost agreeing on who holds what before the first iteration ever runs. Distribution is not a default. It is what you reach for when the data has already won the argument against your RAM. Data stays pinned, and the centroids are the only thing that crosses the wire.
