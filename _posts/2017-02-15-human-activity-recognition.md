---
layout: single
title: "Random Forest feature selection - human activity recognition from phone sensors"
date: 2017-02-15T15:39:55-04:00
last_modified_at: 2017-02-15T15:39:55-04:00
categories: [blog]
...
header:
  overlay_image: /images/blog/headers/human-activity-recognition.jpg
  overlay_filter: 0.4
  teaser: /images/blog/headers/human-activity-recognition.jpg
---
...
When I started this project I assumed I would need most of the 561 features the dataset hands you. I was wrong about how few it takes. A phone clipped to your waist can tell walking-upstairs from sitting at 94.37% accuracy on the held-out test records, and the final model gets there on 5 of those 561 numbers. The other 556 I threw away, and the accuracy barely moved. I did not expect that going in. So I worked my way down the dependency chain to understand it: what the raw signal actually carries, then which of its features survive a hard cut, then which model reads the survivors best, and finally where the whole thing would stop holding.
...
<center>
<figure class="full">
<img src="/images/blog/activityRecognition/image9.png" alt="Distribution of tBodyAccJerk-std()-X across the six activity categories">
<figcaption>Distribution of <code>tBodyAccJerk-std()-X</code>, one representative feature, across the six activities. The still postures pile up at the low end while the walking activities spread higher, splitting the six into two broad groups and hinting that a small handful of features might carry the classification.</figcaption>
</figure>
</center>
...
First I had to split the data. I divided it into train and test sets in a 7:3 ratio by random sampling without replacement, one random draw of 70% of the rows, so each set stays representative of the whole. Sampling per output class instead made no meaningful difference here. To keep everything reproducible I set `random_state=42` at the top, which fixes the train and test sets and the feature draws on every run.

One caveat I owe you before the numbers land, because it changes how to read them. That draw is by record, not by subject. Each record is a 2.56-second window of sensor readings, and consecutive windows overlap by 50%, so neighboring records share half their raw samples. Split at random, a near-twin of a test record can end up in the training set, and windows from the same volunteer land on both sides of the split. The UCI project ships a subject-wise split for exactly this reason, 21 volunteers for training and 9 held out, so that no one's records cross the boundary. My random split ignores that boundary, so the test accuracy is best read as accuracy on held-out records from these same 30 people, not accuracy on someone the model never met. A subject-wise split would almost certainly score lower; I did not re-run it, so I will not put a number on that gap. What it would take to trust the model on a genuinely new person is the note I end on.
...
The two train-row numbers are not the same kind of measurement. The Random Forest train cell is the out-of-bag score: the rows left out of each tree's bootstrap, scored as an internal held-out set, so it is not an in-sample number. The SVM train cell is resubstitution accuracy, the model graded on the same rows it was fit on. So the fair Random Forest comparison is OOB against test, and those two land almost on top of each other.

The four numbers fall out of a short scikit-learn path:

```python
# record-level split: a random 70% of the rows, not grouped by subject
train = data.sample(frac=0.7, random_state=42)
test  = data[~data.index.isin(train.index)]

feats = ['angle(X,gravityMean)', 'tGravityAcc-mean()-Y',
         'tGravityAcc-min()-X', 'tGravityAcc-max()-X', 'tBodyAcc-mad()-X']

rf = RandomForestClassifier(n_estimators=25, oob_score=True, random_state=42)
rf.fit(train[feats], train.activity)

rf.oob_score_                                            # 0.9450  RF train (oob)
accuracy_score(test.activity, rf.predict(test[feats]))   # 0.9437  RF test
```

The SVM row comes from the same `accuracy_score` call on the train and test frames. Random Forest wins by about 12 points on the test set. What reassured me more than the win is the consistency: the OOB estimate (94.50%) almost exactly matches the held-out test accuracy (94.37%), which says the 5-feature model is not memorizing individual training rows. It does not, on its own, say the model would hold up on a volunteer it never trained on. The record-level split leaves that question open, and I come back to it at the end.

To see where the remaining errors actually live, I looked at the confusion matrices. Laying and the motion activities are clean to tell from the still postures, since orientation alone separates posture from movement. The errors that survive sit within a group, among the activities that look alike to the sensors. Sitting versus standing is the worst of them: on the Random Forest test set, 21 standing windows are read as sitting and 17 sitting windows as standing, so that single pair accounts for 38 of the model's errors, more than any other. Both are upright and still, which is exactly why they blur.
...
<figcaption>Confusion matrices on the test set, Random Forest (left) and SVM (right). The off-diagonal mass clusters among the similar activities rather than scattering across all six.</figcaption>
</figure>
</center>

The same separability shows up in the selected feature space itself. A pairplot of the 5 chosen features, colored by activity, sorts laying into its own corner and separates the still postures from the walking ones, while the three walking activities pile onto each other and sitting overlaps standing, the same structure the confusion matrices show. It was reassuring to find the model's errors and the feature scatter telling one story.

<center>
<figure class="full">
<img src="/images/blog/activityRecognition/image11.png" alt="Pairwise scatter matrix of the five selected features, points colored by the six activities">
<figcaption>Pairplot of the five selected features, colored by activity. Laying separates cleanly and the still postures pull away from the walking ones, while the three walking activities overlap among themselves and sitting overlaps standing, mirroring where the confusion matrices put the errors.</figcaption>
</figure>
</center>
...
Two limits sit on top of each other here. The first I already flagged: because the split is by record, the tight 94.50% to 94.37% gap is a within-population result, held-out windows from the same 30 people, so it does not yet speak to a new person. The second is the one I built in by choosing Random Forest. I leaned on the assumption that random forests do not usually overfit, and that tight gap is what the assumption looks like when it holds at the record level. It breaks down when the training data is heavily biased, and this dataset is relatively balanced across activities and subjects, so the assumption holds here. Move to a population that skews one way, an age band these 30 volunteers never covered, a gait the sensors never saw, and the no-overfit comfort is the first thing I would stop trusting.
