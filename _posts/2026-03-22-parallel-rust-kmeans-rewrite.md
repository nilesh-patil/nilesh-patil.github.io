---
layout: single
title: "parallel Rust k-means - benchmarking a clustering rewrite against NumPy and scikit-learn"
...
header:
  overlay_image: /images/blog/headers/parallel-rust-kmeans-rewrite.jpg
  overlay_filter: 0.4
  teaser: /images/blog/headers/parallel-rust-kmeans-rewrite.jpg
---

Open the [live demo](https://www.nilesh42.science/bench-kmeans-rust/demo/), pick a point distribution, and hit run. The first pass clusters your points with the hand-rolled Rust k-means from this post, compiled to WebAssembly; the second is a pure-JavaScript k-means on the *same* data and the *same* seed, timed beside it with `performance.now()` on your machine. That gap is what this whole post measures. At n=1,000, a full elbow sweep (load the CSV, fit every k from 1 to K, write results) takes pure-Python NumPy 0.43 s and scikit-learn 1.57 s; my hand-rolled Rust binary does it in 0.029 s, roughly 15x over NumPy and 54x over the industrial, BLAS-backed scikit-learn. That 15x is the small-n, single-run figure; the paired median across the n=1k cells lands at 14.5x. Then I gave everyone the same initialization, swept out to a quarter-million points, and watched the 15x shrink to 3.5x, with one corner of the grid flipping outright.

There is only **one computation** in this whole investigation: Lloyd's k-means loop, assign then update, on a matched k-means++ grid. I implemented it four times, and each implementation is best read as a single diff from the one before it. Speed, parallelism, memory, accuracy, and the model's hard limits are not five separate experiments. They are five lenses on one shared substrate. Every interesting result is what changes, and what doesn't, as you walk the diffs.
...
<figure>
  ...
  <figcaption>Median end-to-end runtime vs dataset size. Pooled over the whole grid (left panel) Rust is the lowest line at every size, 256k included. scikit-learn overtakes it only in the heaviest corner (right panel, 32 features, k_max=32), one of just 4 cells out of 54 where scikit-learn wins. The crossover is a corner case, not the general rule.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Rayon speedup over serial Rust by thread count, one curve per dataset size (medians of 3, k_max=8). The curve plateaus near 1.3x at large n and drops below parity at small n.</figcaption>
</figure>

It told me the same loop runs barely faster on 14 cores than on one. Peak speedup climbs with n and plateaus near 1.3x, reaching 1.318x at n=256k and 1.286x at n=64k. At the heaviest workload it's concrete: 256k/32/k=32 finishes in 16.23 s parallel against 20.76 s serial, about 1.28x. And at small n the threads turn harmful: n=1k peaks at 1.09x with a single worker and falls to 0.725x at 14 threads, a 28% penalty for spinning up all the cores.

Two things hold the gain near 1.3x. The elbow sweep fits k = 1..K, so most fits are small-k with little arithmetic per point, too little to amortize Rayon's split/join overhead. And the data is an array of structs (`Vec<DataPoint>`, each point owning a heap-allocated `Vec<f64>`), so the inner loop pointer-chases and goes memory-bandwidth-bound: a handful of cores saturate the bus, and the rest only add coordination cost. This is why parallel Rust's grid-wide median wall time (0.197 s) barely edges serial (0.201 s). Across most of the grid the threads add nothing, and on the smallest cells they cost. The 5.1x-vs-Python paired figure is mostly the serial 4.5x with the modest large-n thread gain riding on top.
...
Switch the lens from time to space and the same four implementations re-rank in a way that, for once, never flips with size. Median sampled peak RSS per 1,000 rows: serial Rust 0.61 MB, parallel Rust 0.73 MB, NumPy 7.41 MB, scikit-learn 12.63 MB. That puts serial Rust about 12x leaner than NumPy (7.41/0.61) and 21x leaner than scikit-learn (12.63/0.61). Parallel Rust carries a small premium for per-thread bookkeeping but stays in the same league. Unlike the speed gap, no size flips this ranking.
...
The concrete bill shows up in the heaviest workload: at 256k×32, k=32, scikit-learn peaks at 795 MB and NumPy at 924 MB, while both Rust builds sit near 190 MB (190 parallel, 194 serial). Rust never builds the $n \times k$ distance matrix at all: its assignment step walks each point against the centroids holding only a running nearest-index, so peak memory tracks the data itself rather than a float64 block that reallocates every iteration.
...
scikit-learn keeps its one edge in that worst case through its greedy k-means++ variant, which tries several candidate seeds at each step and keeps the best. (Convergence tolerance and empty-cluster handling differ too, so the gap isn't all seeding.) But the seeding is also where the cheapest win in the whole project lives, and it is available to all four. k-means++, due to Arthur and Vassilvitskii in 2007, changes only the seeding, not a line of the loop. Rather than dropping k seeds uniformly at random, let $D(x)$ be the distance from $x$ to the nearest seed already chosen; then pick each new seed with probability $p(x)$ proportional to $D(x)^2$:
...
| Situation | Reach for |
|---|---|
| Small/medium NumPy k-means bottleneck | Serial Rust: ~4.5x faster, ~12x leaner |
