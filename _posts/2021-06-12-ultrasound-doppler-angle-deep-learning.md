---
layout: single
title: "Reading the Doppler angle off a single B-mode image"
...
[Code](https://github.com/nilesh-patil/ultrasound-doppler-angle-estimation) &middot; [Paper &mdash; EMBC 2019](https://arxiv.org/abs/2508.04243) &middot; [Project site](https://www.nilesh42.science/ultrasound-doppler-angle-estimation/)
</div>

## Introduction

**Problem:** A spectral-Doppler study reports a blood velocity, but that number is only as trustworthy as one angle a sonographer sets by hand on every exam: the angle between the ultrasound beam and the direction of flow. A few degrees off near the steep part of the curve, and the reported velocity is wrong by tens of percent &mdash; enough to push a borderline carotid stenosis into the wrong grade. It is a leading, operator-dependent source of velocity error, and accreditation reviewers flag improper angle correction in a large share of vascular-lab applications.

**Core Idea:** Read the angle off the image instead of asking the operator to set it. [Patil &amp; Anand (EMBC 2019)](https://doi.org/10.1109/EMBC.2019.8857587) showed that a convolutional network can regress the Doppler angle straight from a single grayscale B-mode frame of the carotid &mdash; no color Doppler, no segmentation, no hand-placed landmarks &mdash; and reported under 3&deg; mean error. I first-authored that paper.

**This project:** Seven years on, I rebuilt the pipeline from scratch on modern infra and tooling, and carried it end to end past where the original two pages could stop. The rest of this post is that build: the data and labels, the one pooling choice that makes a frozen backbone work, two evaluation protocols, how far tuning and ensembling climb, and what a clinic would still need. Three questions anchor it:

- Does a from-scratch build hold the paper's accuracy?
- *Why* does a frozen ImageNet backbone work at all on 84 images?
- How far does the estimator go once it is tuned and calibrated?

Short answers: yes, once one pooling choice is fixed; because the vessel's orientation is already in the frozen features if you do not average it away; and down to about 2&deg; within-population, with calibrated intervals for the clinic. The interactive write-up lives on the [project site](https://www.nilesh42.science/ultrasound-doppler-angle-estimation/); the rest of this post walks the build that gets there.
...
Near 60 degrees, each extra degree of angle moves the reported velocity by about 3%. A model that reads the angle off the image has to be accurate to within a degree or so to be worth anything.
...
| | |
|---|---|
| The core model | A *frozen* DenseNet201 plus a small head lands at **5.84% MAPE** (3.77&deg; MAE, $R^2$ 0.982) &mdash; the paper's best single-model regime, with no fine-tuning, once the pooling is fixed. |
| The pooling insight | Global average pooling is approximately rotation-invariant, the wrong bias for an orientation target. An orientation-preserving **grid pooling** head lifts the same frozen backbone from about 14% to 5.84% MAPE with no fine-tuning. |
...
## The model: it's the pooling, not the backbone {#pooling}

The whole project rests on one head choice, so it is worth seeing it fail first. My first frozen-feature runs landed near 14% MAPE (mean absolute percentage error). The obvious read is that frozen features simply can't reach fine-tuned accuracy. That read is wrong.
...
Grid pooling keeps it. Instead of collapsing to one vector, average-pool the final feature map onto a coarse $G \times G$ grid, then flatten. A top-left cell is no longer interchangeable with a bottom-right one, so the flattened vector carries coarse spatial location and the head can read orientation off it. On a single augmented split, frozen DenseNet201 with grid pooling reaches 5.84% MAPE, 3.77&deg; MAE, $R^2$ 0.982 &mdash; the same regime as the EMBC-2019 best single model (about 2.87&deg; / 4.03%), and reached here with no fine-tuning at all.[^grid]

A frozen ImageNet backbone already sees the vessel orientation; the work is in not pooling it away.

## Rotation manufactures the labels {#labels}

The cohort has no clinical angle annotation, so $\theta$ was hand-drawn once per image in a MATLAB GUI. That single reading per base image is the only ground truth. To turn 84 readings into a regression corpus, each base image is rotated through $[-60°, +60°]$ in 5-degree steps, 25 oriented views apiece, for $84 \times 25 = 2{,}100$ labelled views. The label of a rotated view is exact: the new angle is the base angle plus the rotation. Because the increment is exact, the network learns to read relative image orientation, and the rotation sweep does double duty as augmentation and label source.

Those 2,100 views are not 2,100 independent observations. They are 25 geometrically coupled copies of 84 base images drawn from about 10 volunteers, a three-level hierarchy of volunteer, base image, and rotated view. That coupling is why there are two ways to score the model, not one.
...
Image-level sampling is the paper's protocol: split at random over the 2,100 augmented rows. It measures how well the estimator interpolates across the full population of orientations. Patient-level sampling holds out whole volunteers using GroupKFold over patient id, so the test anatomy is unseen at any rotation. It is the stricter, cross-subject lens, and the one a clinic cares about.
...
## How far the estimator climbs {#climb}

Newer encoders are not better here. A frozen-feature bake-off across the modern zoo leaves DenseNet201 (14.13% patient MAPE) below ConvNeXt-Base (15.65%), ConvNeXt-Tiny (16.07%), and the EfficientNet / V2 family (roughly 17-21%). With only 84 base images, the ranking is empirical: these frozen feature spaces happen to encode the orientation cue better, and the newer encoders do not buy anything here.
...
<figure>
  <svg viewBox="0 0 680 365" role="img" aria-labelledby="bake-t bake-d" style="width:100%;height:auto;max-width:680px;color:inherit" xmlns="http://www.w3.org/2000/svg">
    ...
    <g fill="currentColor" font-family="-apple-system, system-ui, sans-serif">
      ...
      <text x="168" y="347" font-size="11" opacity="0.7">patient-level MAPE (%), frozen features, 5-fold CV &mdash; lower is better</text>
    </g>
  </svg>
...
From that core model, an Optuna TPE search over the head and optimizer, run on cached frozen features, moves single-model DenseNet201 from 4.58 to 4.03% image-level and 12.59 to 10.80% patient-level: a real but small gain.[^insample] Each trial is a shallow head fit, one feature extraction per backbone serves both protocols, and no GPU is needed.

Stacking the five tuned backbones is where the accuracy actually moves. A Ridge meta-learner on pooled out-of-fold predictions reaches 2.79% / 1.96&deg; image-level and 8.53% / 5.93&deg; patient-level, the first configuration below 10% MAPE cross-patient. Read against its like-for-like predecessor &mdash; the single tuned DenseNet201 on pooled OOF (7.80&deg; / 10.14% patient-level) &mdash; the genuine ensembling gain is about 1.9&deg;. The plain mean of the same five lands at 9.89% patient-level. Before tuning, that mean was a useless 21.9%; tuning calibrated the members just enough for averaging to help at all.
...
## What a clinic would actually need {#clinical}

A clinic acts on a number with a band, so three post-hoc checks on the held-out patient-level predictions say how far this is from usable.

Split-conformal calibration, with a patient-disjoint calibration set, gives a 90% band of &plusmn;20.5&deg; at 95.2% empirical coverage. That is wide &mdash; roughly 41 degrees of total width &mdash; the price of distribution-free validity on a cohort this size. A conformal interval is a prediction band built to cover the truth at a stated rate without assuming a noise model; here coverage at or above nominal means the band is not optimistic. The 95.2% figure is a single seed-42 patient-disjoint split, so it is itself a point estimate, and the guarantee is marginal exchangeability, not per-patient conditional coverage.
...
<figure>
  ...
  <figcaption>Every point sits on or above the diagonal &mdash; empirical coverage meets or beats the nominal level at 80, 90, and 95%. The bands are wide (the 90% band is &plusmn;20.5&deg;) &mdash; that width is the price of distribution-free validity on a cohort this size.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Look for the warm band hugging the longitudinal vessel wall, not the speckle field around it. The heat lands on the boundary that fixes the flow axis. Three hand-picked frames, illustrative rather than a cohort-wide attribution study.</figcaption>
</figure>

Grad-CAM, finally, puts the attention on the carotid wall, the boundary that fixes the flow axis, rather than on speckle or label burn-in. The network keys on anatomy, shown here on three hand-picked frames.

## Where the ceiling is {#limits}

I hit a hardware ceiling, not a tuning one. End-to-end fine-tuning of the strong backbones OOMs the Apple GPU even at batch 16, and a from-scratch CNN fails outright (MAPE around 101%, negative $R^2$). For 84 images, those two failures are positive evidence that frozen transfer is the right tool here.

What remains is concrete. Modern self-supervised encoders, DINOv2 or a medical-ultrasound foundation model, need a CUDA box and a reworked dependency stack. A hand-crafted structure-tensor baseline (MAE 3.16&deg;) with a learned-plus-classical circular fusion (2.72&deg;) suggests the learned and geometric cues capture partly complementary signal, though both figures are in-sample on a narrow base-angle band.

Everything traces back to `results/`. The library is typed and test-driven; every figure regenerates from script at seed 42, nothing hand-typed.
...
[Try the angle dial](#dop-svg){: .btn-soft .btn-soft--primary} [Project site](https://www.nilesh42.science/ultrasound-doppler-angle-estimation/){: .btn-soft} [Project write-up]({{ '/portfolio/ultrasound-doppler-angle-estimation/' | relative_url }}){: .btn-soft} [Browse the code](https://github.com/nilesh-patil/ultrasound-doppler-angle-estimation){: .btn-soft}
</div>
...
[^insample]: These tuned single-model figures are in-sample to the search (selected and reported on the same folds), so they are optimistic. They appear only as the like-for-like predecessor of the ensemble.
[^anchor]: Both lines mix estimators by stage. On the image line, the 5.84% core-model point is the single augmented-split figure while tuned (4.03%) and ensemble (2.79%) are CV / pooled-OOF means; on the patient line, the core-model and tuned points use the per-fold-mean DenseNet201 (12.59, then 10.14 on pooled OOF) and the ensemble uses pooled OOF. The Ridge stacker is fit on OOF over only 12 groups, so stacking-on-OOF is itself mildly optimistic; no error-correlation analysis is claimed.
[^tta]: This 7.80&deg; base-image footing is the same residual as the 10.14% pooled-OOF patient number above, viewed per base image rather than per patient. It is a different population from the 5.93&deg; patient-level ensemble headline, so the TTA before/after is not directly comparable to that headline.
