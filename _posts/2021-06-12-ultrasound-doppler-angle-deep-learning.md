...
math: true
header:
  overlay_image: /images/blog/headers/doppler.jpg
  overlay_filter: 0.4
  teaser: /images/blog/headers/doppler.jpg
---
...
[Code](https://github.com/nilesh-patil/ultrasound-doppler-angle-estimation) &middot; [Preprint](https://arxiv.org/abs/2508.04243) &middot; [EMBC 2019 paper](https://doi.org/10.1109/EMBC.2019.8857587) &middot; [Project site](https://www.nilesh42.science/ultrasound-doppler-angle-estimation/)
</div>
...
| | |
|---|---|
| The core model | A *frozen* DenseNet201 plus a small head lands at **5.84% MAPE** (3.77&deg; MAE, $R^2$ 0.982) - the paper's best single-model regime, with no fine-tuning, once the pooling is fixed. |
...
## Stage one, the labels: rotation manufactures the corpus {#labels}
...
Those 2,100 views are not 2,100 independent observations. They are 25 geometrically coupled copies of 84 base images drawn from about 10 volunteers, a three-level hierarchy of volunteer, base image, and rotated view. That coupling matters later. It is why there are two ways to score the model, not one.

## Stage two, the model: it is the pooling, not the backbone {#pooling}

The whole project rests on one head choice, so look at it fail first. My first frozen-feature runs landed near 14% MAPE (mean absolute percentage error). The obvious read is that frozen features simply cannot reach fine-tuned accuracy. That read is wrong.
...
Grid pooling keeps it. Instead of collapsing to one vector, average-pool the final feature map onto a coarse $G \times G$ grid, then flatten. A top-left cell is no longer interchangeable with a bottom-right one, so the flattened vector carries coarse spatial location and the head can read orientation off it. On a single augmented split, frozen DenseNet201 with grid pooling reaches 5.84% MAPE, 3.77&deg; MAE, $R^2$ 0.982 - the same regime as the EMBC-2019 best single model (about 2.87&deg; / 4.03%), and reached here with no fine-tuning at all.[^grid] A frozen ImageNet backbone already sees the vessel orientation. The work is in not pooling it away.

## Stage three, the protocol: two ways to be right {#two-protocols}

That core model now has a number on it. Which number depends on how you split the corpus, and the hierarchy from stage one forces the question.
...
So the gains had to come from the head, not the encoder. An Optuna TPE search over the head and optimizer, run on cached frozen features, moves single-model DenseNet201 from 4.58 to 4.03% image-level and 12.59 to 10.80% patient-level: a real but small gain.[^insample] Each trial is a shallow head fit, one feature extraction per backbone serves both protocols, and no GPU is needed.
...
Grad-CAM, finally, puts the attention on the carotid wall, the boundary that fixes the flow axis, rather than on speckle or label burn-in. The network keys on anatomy, shown here on three hand-picked frames. The pipeline is whole: rotation-built labels, a frozen backbone read through grid pooling, scored two ways, tuned and stacked, then wrapped in a calibrated band and an attribution check.
...
The ceiling I actually hit is hardware, not tuning. End-to-end fine-tuning of the strong backbones OOMs the Apple GPU even at batch 16, and a from-scratch CNN fails outright (MAPE around 101%, negative $R^2$). For 84 images, those two failures are not setbacks; they are positive evidence that frozen transfer is the right tool here. The small-data regime was telling me what it wanted.

Everything traces back to `results/`. The library is typed and test-driven; every figure regenerates from script at seed 42, nothing hand-typed. The proof is in the repo. The post is the explanation.
